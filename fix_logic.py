...
# Define the replacement block containing the grouped logic and the unified logic
new_block = """        async function loadAndGroupAllProductsDesktop(isLoadMore = false) {
        ...
        async function loadProducts(isLoadMore = false) {
            ...
            if (!isLoadMore) {
                mainContentArea.innerHTML = `<p class="col-span-full text-center text-gray-500 text-lg">Loading...</p>`;
                lastVisibleProduct = null;
                hasMoreProducts = true;
            }
            if (!hasMoreProducts) return;
            ...
            try {
                if (!isLoadMore) {
                    ...
                    if (isMobileView()) {
                        ...
                        const productGrid = document.createElement('div');
                        productGrid.id = 'filtered-product-grid';
                        productGrid.className = 'grid grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-10 mt-8';
                        mainContentArea.appendChild(productGrid);
                    }
                }
                ...
                let constraints = [];
                if (currentFilters.brands && currentFilters.brands.length) constraints.push(where("brand", "in", currentFilters.brands));
                if (currentFilters.categories && currentFilters.categories.length) constraints.push(where("category", "in", currentFilters.categories));
                if (currentFilters.sizes && currentFilters.sizes.length) constraints.push(where("sizes", "array-contains-any", currentFilters.sizes));
                ...
                if (querySnapshot.empty) {
                    hasMoreProducts = false;
                    if (!isLoadMore) productGrid.innerHTML = '<p class="col-span-full text-center text-gray-500">No products found.</p>';
                } else {
                ...
                if (!isLoadMore) {
                    ...
                    if (productGrid) {
                        ...
                        mainContentArea.innerHTML = '<p class="text-center text-red-500">Failed to load products.</p>';
                    }
                }
            } finally {
                isLoadingMore = false;
...
pattern = re.compile(r'async function loadProducts\(isLoadMore = false\) \{.*?(?=function generateStars)', re.DOTALL)

# Inject the new block
content = pattern.sub(new_block + "\n\n        ", content)

# Also add the infinite scroll event listener if not present
scroll_listener_code = """
            // Auto infinite scrolling logic
...
if "window.addEventListener('scroll'" not in content:
    content = content.replace("        });\n    </script>\n</body>", scroll_listener_code + "        });\n    </script>\n</body>")

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Restored beautiful category logic and injected infinite scrolling successfully.")
